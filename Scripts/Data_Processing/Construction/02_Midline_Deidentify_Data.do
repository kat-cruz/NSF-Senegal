...
*** get midline individual IDs and keep only IDs and household ID long *** 
use "$data_identified/Midline_Individual_IDs.dta", clear 

keep hhid individ add_new_ hh_full_name_calc_ hh_gender_ hh_age_ hh_relation_with_
...
forvalues i = 1/57 {
	...
	tostring pull_hh_individ_`i', replace 
}

reshape long pull_hh_individ_ add_new_ hh_full_name_calc_ hh_gender_ hh_age_ hh_relation_with_, i(hhid) j(individualindex)

drop if hh_gender_ == . & hh_age_ == . & hh_relation_with_ == . 

merge m:m hhid add_new_ hh_full_name_calc_ hh_gender_ hh_age_ hh_relation_with_ using `individs'
...
merge 1:1 hhid using `individswide'
