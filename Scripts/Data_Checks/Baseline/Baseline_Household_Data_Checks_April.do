*** DISES Baseline Data Checks - Household Survey, April Collection***
*** File Created By: Molly Doruska ***
*** File Last Updated By: Molly Doruska ***
*** File Last Updated On: April 25, 2024 ***
...
global village_observations "$master\Data_Quality_Checks\April_Output\Baseline_Village_Household_Identifiers"
global household_roster "$master\Data_Quality_Checks\April_Output\Baseline_Household_Roster"
global knowledge "$master\Data_Quality_Checks\April_Output\Baseline_Knowledge"
global health "$master\Data_Quality_Checks\April_Output\Baseline_Health" 
global agriculture_inputs "$master\Data_Quality_Checks\April_Output\Baseline_Agriculture_Inputs"
global agriculture_production "$master\Data_Quality_Checks\April_Output\Baseline_Agriculture_Production"
global food_consumption "$master\Data_Quality_Checks\April_Output\Baseline_Food_Consumption"
global income "$master\Data_Quality_Checks\April_Output\Baseline_Income"
global standard_living "$master\Data_Quality_Checks\April_Output\Baseline_Standard_Living"
global beliefs "$master\Data_Quality_Checks\April_Output\Baseline_Beliefs" 
global public_goods "$master\Data_Quality_Checks\April_Output\Baseline_Public_Goods"
global enum_observations "$master\Data_Quality_Checks\April_Output\Baseline_Enumerator_Observations"

*** prep existing household IDs for consistency ***
*import excel "$village_observations\HouseholdIDs_042324.xlsx", clear first

*save "$village_observations\HouseholdIDs_042324.dta", replace 

*** Import data - update this every new data cleaning session ***
import delimited "$data\DISES_enquete_ménage_FINALE_V2_WIDE_26April24.csv", clear varnames(1) bindquote(strict)
...
forvalues i = 1/18 {
	...
	*** drop no consent households *** 
	...
	egen hh_13_`i'_total = rowtotal(hh_13_`i'_1  hh_13_`i'_2  hh_13_`i'_3 hh_13_`i'_4  hh_13_`i'_5)
...
forvalues i = 1/18 {
	...
	*** drop no consent households *** 
	...
	egen hh_21_`i'_total = rowtotal ( hh_21_o_`i' hh_21_`i'_1  hh_21_`i'_2  hh_21_`i'_3  hh_21_`i'_4  hh_21_`i'_5)
...
preserve
    ...
    keep if  `var' != . & `var' < 0 & `var' > 1000000000
	generate issue = "Unreasonable value"
	generate issue_variable_name = "`var'"
	rename `var' print_issue 
	tostring(print_issue), replace
...
preserve
	...
	rename health_5_12_`i' print_issue 
	tostring(print_issue), replace
	keep hhid_village hhid sup enqu sup_name enqu_name hh_phone hh_head_name_complet hh_name_complet_resp issue_variable_name issue print_issue
	if _N > 0 {
		save "$health\Issue_health_5_12_`i'", replace
	}
...
preserve 
	...
	tostring(agri_income_31_o), replace 
