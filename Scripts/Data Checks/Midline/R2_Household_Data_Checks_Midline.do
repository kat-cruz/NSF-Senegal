...
// UPDATE VARIALE NAME	capture label variable "Cette personne fait-elle toujours partie du ménage ? "
// UPDATE VARIALE NAME	capture label variable "Pourquoi cette personne n'est-elle pas dans le ménage ?"
// UPDATE VARIALE NAME	capture label variable "Y a-t-il d'autres personnes dans le ménage ?"
...
//	capture label variable hh_33_`i' "En ce qui concerne les autres eleves de sa classe, pensez-vous que la performance academique de [hh_full_name_calc] est inferieure a celle de la plupart des eleves, a peu pres la meme que celle de la plupart des eleves, ou superieure a celle de la plupart des eleves"
...
**# Bookmark #8
	*capture label variable hh_47_`i' "Combien a été dépensé pour l'éducation de [NOM] au cours des 12 derniers mois par le ménage, la famille et les amis ?"
...
capture label variable agri_income_28_1 "En argent"
...
forvalues i = 1/55 {
  ...
  * check that we have new names for new members 
...
/*
	preserve
			...
			keep villageid hhid sup enqu sup_name enqu_name hh_phone hh_name_complet_resp hh_08_* _household_roster_count still_member_* hhid consent hh_age_*
			drop still_member_w*
		   
			reshape long still_member_ hh_08_ hh_age_, ///
						i(hhid) j(id)

			*** drop no consent households *** 
			drop if consent == 0 
			drop if still_member_ == 0
			

			
			gen ind_var = 0 
			replace ind_var = 1 if hh_08_ < 0 & hh_08_ != -9
			replace ind_var = 1 if hh_08_ > 168
			forvalues i = 1/57 {
			replace ind_var = 0 if _household_roster_count < `i'
			}

			drop if hh_age_ == . & missing(hh_08_)
			keep if ind_var == 1 
			reshape wide still_member_ hh_08_, i(hhid) j(id)
			
			generate issue = "Unreasonable value"
			generate issue_variable_name = "hh_08_"
			rename hh_08_ print_issue 
			tostring(print_issue), replace
			keep villageid hhid sup enqu sup_name enqu_name hh_phone hh_name_complet_resp issue_variable_name issue print_issue
