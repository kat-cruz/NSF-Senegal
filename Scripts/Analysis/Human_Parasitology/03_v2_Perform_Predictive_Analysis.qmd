...
var_labels_latex <- kable(var_labels, format = "latex", booktabs = TRUE,
...
# Write to .tex file
writeLines(var_labels_latex, "C:/Users/km978/Downloads/SenegalGIT/NSF-Senegal/Tables_Figures/Parasitological_Predictive_Analysis/best_predictors.tex")
...
confusion_matrix_threshold <- kable(results_df, format = "latex", booktabs = TRUE, 
  ...
  rrow_spec(7, background = "highlightyellow")
...
confusion_matrix_threshold <- kable(
  results_df,
  format = "latex",
  booktabs = TRUE,
  caption = "Confusion Matrix for sm\\_sh\\_inf at each threshold"
...
# Add manual stripes: even rows light gray, odd rows white
for(i in 1:n){
  if(i %% 2 == 0){
    confusion_matrix_threshold <- confusion_matrix_threshold %>%
      row_spec(i, background = "gray!20")  # light gray stripe
  } else {
    confusion_matrix_threshold <- confusion_matrix_threshold %>%
      row_spec(i, background = "white")    # white stripe
  }
}
