...
run_regressions <- function(data, treatment_col, village_col, vars_to_test) {
  ...
  for (var in vars_to_test) {
    ...
    for (arm in treatments) {
      ...
      # Extract control mean (intercept)
      control_mean <- round(model_summary$coefficients[1, 1], 2)  # Extract intercept (Control mean)
      
      # Extract standard deviation of the control mean (intercept SE)
     # Extract SE of the intercept (Control Group)
      control_se <- round(clustered_se[1, 2], 2)  # SE is in the second column of clustered_se
  ...
  # Extract intercept standard error
      ...
      results <- append(results, list(data.frame(
        ...
        control_mean = control_mean,          # Control mean (intercept)
        control_se = control_se               # Standard deviation (SE of intercept)
      )))
    }
  }
...
#       clustered_se <- coeftest(model, vcov = vcovCL(model, cluster = subset[[village_col]]))
#       
#       # Extract F-statistic from the model summary
...
#         stat = round(clustered_se[2, 1], 2),  # Coefficient (estimate)
#         p = round(clustered_se[2, 4], 3)     # p-value
#         #f_stat = round(f_statistic, 2)        # F-statistic
#       )))
#     }
#   }
#   
...
# Run regressions for the baseline variables
 ...
 vars_to_test_reg
    regression_results <- run_regressions(balance_df, 'treatment_group', 'hhid_village', vars_to_test_reg)
...
final_table %>%
 ...
 general = "Reported statistics are regression coefficients, with p-values in parentheses and false discovery rate (FDR)-adjusted q-values in brackets. Standard errors are clustered at the village level. P-values indicate statistical significance: * denotes significance at the 5% level (p ≤ 0.05), and ** denotes significance at the 1% level (p ≤ 0.01).",
  ...
  save_kable("balance_tablev9.html")
