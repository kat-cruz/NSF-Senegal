...
1) There's a function that handles variables with indices, and one that handles variables without indices, and these functions generate summary stats for each of the different types of variables.These are distinguished by the _l for long at the end of each function name.
...
2) Run each code chunk to compute the summary statistics of each variable. I've separated them out for ease of copy and pasting, and to keep a precise record of which variables we have currently. 
