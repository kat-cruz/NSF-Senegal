...
symbol = c(
  ...
  save_kable("baseline_summary_statsV5.html")
...
final_table %>%
  ...
  save_kable("balance_tablev_jointf_5.html")
...
index_description <- "
<b><u>Note on Asset Index Creation:</u></b><br><br>
The asset index was created using Principal Component Analysis (PCA). The first principal component (PC1) was extracted from a set of variables, including information on physical assets, living conditions, and household characteristics. The index represents the variation in these factors, and is often interpreted as a measure of wealth or socioeconomic status. The asset index was then standardized (asset_index_std) to allow for easier interpretation and comparison across households.
"
...
# Create HTML output with table and note
...
html_content <- paste0(index_description, as.character(html_output))

# Specify the file to save the HTML output
output_file <- "asset_index_summary_results.html"
