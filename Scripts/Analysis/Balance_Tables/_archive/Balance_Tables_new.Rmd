...
####Filter data and transform to long TO UPDATE
...
# Save to Excel
write_xlsx(summary_table_with_footnotes, "_baseline_summary_statsV7.xlsx")
...
###non-robust f-stats for comparison
run_all_regressions <- function(data, treatment_col, village_col, vars_to_test) {
  results_list <- list()
  ...
  for (var in vars_to_test) {
    ...
    subset <- data %>%
      select(all_of(var), all_of(treatment_col), all_of(village_col))
    ...
    f_stat <- round(fstats["value"], 2)
    f_pval <- round(pf(fstats["value"], fstats["numdf"], fstats["dendf"], lower.tail = FALSE), 3)

    # Extract N used in the model
    n_obs <- nobs(model)

    # Extract control stats
    control_mean <- round(clustered_se[1, 1], 2)
    control_se <- round(clustered_se[1, 2], 2)
...
#     f_stat <- round(summary(model)$fstatistic[1], 2)
# 
#     # Sample size
#     n_obs <- nrow(subset)
# 
#     # Control group stats (intercept)
#     control_mean <- round(clustered_se[1, 1], 2)
#     control_se <- round(clustered_se[1, 2], 2)
# 
#     # Coefficients
...
#       clustered_se <- coeftest(model, vcov = clustered_vcov)
# 
#       # --- 1. F-stat from summary(model)
#       model_summary <- summary(model)
#       f_raw <- round(model_summary$fstatistic[1], 2)
#       p_raw <- round(pf(model_summary$fstatistic[1],
#                         model_summary$fstatistic[2],
#                         model_summary$fstatistic[3],
#                         lower.tail = FALSE), 3)
# 
#       # --- 2. F-stat from Wald test with clustered SE
#       wald_clustered <- waldtest(model, vcov = clustered_vcov, test = "F")
#       f_clustered <- round(wald_clustered[2, "F"], 2)
#       p_clustered <- round(wald_clustered[2, "Pr(>F)"], 3)
# 
#       # --- 3. F-stat from Wald test without clustering
#       wald_unclustered <- waldtest(model, test = "F")
#       f_unclustered <- round(wald_unclustered[2, "F"], 2)
#       p_unclustered <- round(wald_unclustered[2, "Pr(>F)"], 3)
# 
#       # Sample size
#       n_obs <- nobs(model)
# 
#       # Extract control group stats
#       control_mean <- round(clustered_se[1, 1], 2)
#       control_se <- round(clustered_se[1, 2], 2)
# 
#       # Treatment stats
...
#         f_stat_raw = f_raw,
#         p_value_raw = p_raw,
#         f_stat_clustered = f_clustered,
#         p_value_clustered = p_clustered,
#         f_stat_unclustered = f_unclustered,
#         p_value_unclustered = p_unclustered,
#         row.names = NULL
#       )
# 
#       results_list[[var]] <- results
...
#         p_value_raw = NA,
#         f_stat_clustered = NA,
#         p_value_clustered = NA,
#         f_stat_unclustered = NA,
#         p_value_unclustered = NA,
#         row.names = NULL
#       )
#     })
#   }
...
#     
#     # F-test using Wald test
#     wald <- waldtest(model, vcov = clustered_vcov)
#     f_stat <- round(wald[2, "F"], 2)
#     p_value <- round(wald[2, "Pr(>F)"], 2)
#     
#     # Store results in a data frame
#     f_test <- data.frame(variable = var, f_statistic = f_stat, p_value = p_value)
#     
#     results[[var]] <- f_test
#   }
#   
...
# 
# 
# variance_check <- balance_df %>%
#   group_by(treatment_group) %>%
#   summarise(variance = var(hh_15_2, na.rm = TRUE))
# 
# # Print the result
...
#####no clustering, for reference 

```{r}
# run_f_tests_no_clustering <- function(data, treatment_col, vars_to_test) {
#   results <- list()
#   
#   for (var in vars_to_test) {
#     formula <- as.formula(paste(var, "~", treatment_col))
#     
#     # Fit model (no clustering)
#     model <- lm(formula, data = data)
#     
#     # F-test using Wald test (no clustering)
#     wald <- waldtest(model)
#     
...
#     
#     # Store results
#     results <- append(results, list(data.frame(variable = var, f_statistic = f_stat, p_value = p_value)))
#   }
#   
...
# vars_to_test_fstat <- setdiff(names(balance_df), c("hhid", "hhid_village", "treatment_group"))
# 
# f_test_results <- run_f_tests_no_clustering(
#   data = balance_df,
#   treatment_col = "treatment_group",
#   vars_to_test = vars_to_test_fstat
# )
...
final_table %>%
 ...
 general = "Reported statistics are regression coefficients, with p-values in parentheses and false discovery rate (FDR)-adjusted q-values in brackets. Standard errors are clustered at the village level. P-values indicate statistical significance: * denotes significance at the 5% level (p ≤ 0.05), and ** denotes significance at the 1% level (p ≤ 0.01).",
  ...
  save_kable("balance_tablev12.html")
...
total_rows <- nrow(joint_df)
missing_counts <- colSums(is.na(joint_df))
missing_counts
total_rows
# Compute missing percentages correctly
missing_percents <- (missing_counts / total_rows) *100
...
##add clustering at the village level 
view(joint_df)
```
...
vars_to_impute <- c("agri_income_05", "agri_6_15", "hh_age_h", "hh_education_level_bin_h", "hh_education_skills_5_h", "hh_gender_h", "health_5_6_")
...
model <- multinom(formula, data = ml_balance_df)

# Clustered standard errors at the village level
Cluster_model <- cluster.im.mlogit(model, balance_df, ~hhid_village)

# Display summary with clustered SEs
...
# Log-likelihood
log_lik <- logLik(Cluster_model)  # Extract log-likelihood from clustered model

# Degrees of freedom (number of predictors)
df_model <- length(coef(Cluster_model))

# LR Chi-Square (Likelihood Ratio Test Statistic)
null_model <- multinom(treatment_group ~ 1, data = balance_df)  # Fit null model (intercept only)
lr_chi2 <- 2 * (logLik(Cluster_model) - logLik(null_model))  # LR test using clustered model

# p-value for LR test
p_value <- pchisq(as.numeric(lr_chi2), df = df_model, lower.tail = FALSE)

# Pseudo R² (McFadden’s R²)
pseudo_r2 <- 1 - (logLik(Cluster_model) / logLik(null_model))
...
final_table %>%
  ...
  save_kable("balance_tablev_jointf_4.html")
##balance_tablev11.html = without -9s
```
