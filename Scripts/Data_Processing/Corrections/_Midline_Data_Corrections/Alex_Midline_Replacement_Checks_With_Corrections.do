...
* Updates recorded in GitHub: [Alex_Midline_Replacement_Checks_With_Corrections.do](https://github.com/kat-cruz/NSF-Senegal/blob/main/Scripts/Data_Processing/Corrections/_Midline_Data_Corrections/Alex_Midline_Replacement_Checks_With_Corrections.do)
*==============================================================================
*
* Description:
* This script performs data quality checks for the DISES Midline Household Replacement Survey dataset. It verifies completeness, consistency, and correctness of household survey records, largely focusing on skip patterns and other data integrity issues.
* It ensures that values that have been confirmed and corrections are not reflagging as issues.
*
* Key Functions:
...
global village_observations "$master\Data_Management\Output\Data_Quality_Checks\Midline\Midline_Village_Observations\Replacement_Survey"
global household_roster "$master\Data_Management\Output\Data_Quality_Checks\Midline\Midline_Household_Roster\Replacement_Survey"
global knowledge "$master\Data_Management\Output\Data_Quality_Checks\Midline\Midline_Knowledge\Replacement_Survey"
global health "$master\Data_Management\Output\Data_Quality_Checks\Midline\Midline_Health\Replacement_Survey" 
global agriculture_inputs "$master\Data_Management\Output\Data_Quality_Checks\Midline\Midline_Agriculture_Inputs\Replacement_Survey"
global agriculture_production "$master\Data_Management\Output\Data_Quality_Checks\Midline\Midline_Agriculture_Production\Replacement_Survey"
global food_consumption "$master\Data_Management\Output\Data_Quality_Checks\Midline\Midline_Food_Consumption\Replacement_Survey"
global income "$master\Data_Management\Output\Data_Quality_Checks\Midline\Midline_Income\Replacement_Survey"
global standard_living "$master\Data_Management\Output\Data_Quality_Checks\Midline\Midline_Standard_Living\Replacement_Survey"
...
import delimited "$corrected\CORRECTED_DISES_enquete_ménage_FINALE_MIDLINE_REPLACEMENT_WIDE_5Mar2025.csv", clear varnames(1) bindquote(strict)

**************************** rename variables ****************************
* Generate a counter variable within each hhid_village
bysort hhid_village (hhid_village): gen counter = _n

* Construct hh_global_id by concatenating hhid_village with 30, 31, 32...
gen hh_global_id = hhid_village + string(29 + counter)

* Drop temporary variable if not needed
drop counter
...
forvalues i = 1/14 {
    rename hh_full_name_calc_`i' pull_hh_full_name_calc__`i'
}
...
* LARGEST HOUESHOLD RIGHT NOW (FEB 12) is 14 (i = 14)
* 
...
//	drop if add_new_`i' == 0
	...
	egen hh_13_`i'_total = rowtotal(hh_13_`i'_1  hh_13_`i'_2  hh_13_`i'_3) // REMOVED after hh_13_`i'_3 for replacement protocol
	
	gen ind_var = 0 
	replace ind_var = 1 if hh_13_`i'_total > hh_10_`i' 
	replace ind_var = 0 if _household_roster_count < `i' 
	replace ind_var = 0 if key == "uuid:64a3cbc6-e8b8-4e8f-b508-bdb4d8a0abc9"
 replace ind_var = 0 if key == "uuid:64a3cbc6-e8b8-4e8f-b508-bdb4d8a0abc9" 
 replace ind_var = 0 if key == "uuid:10b77926-7c29-4fec-8e2c-a8153d83dfe7" 
...
//	drop if add_new_`i' == 0
	...
	local pull_hh_full_name_calc pull_hh_full_name_calc__`i' 
	rename `pull_hh_full_name_calc' hh_member_name 
	egen hh_21_`i'_total = rowtotal ( hh_21_o_`i' hh_21_`i'_1  hh_21_`i'_2  hh_21_`i'_3 )  //REMOVED after hh_21_`i'_3 a it hasn't been selected
...
preserve
	gen ind_var = 0
    replace ind_var = 1 if knowledge_18 == 1 & missing(knowledge_23)
	keep if ind_var == 1 
...
*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if list_actifs_o == 1 & missing(actifs_o)
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if agri_6_11 == 1 & missing(agri_6_12)
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if agri_6_12_99 == 1 & missing(agri_6_12_o)
...
forvalue i=1/3 {
	...
	*** generate indicator variable *** 
	...
	replace ind_var = 0 if parcelleindex_`i' == . 
	replace ind_var = 0 if key == "uuid:255cbd5b-d7d5-4d66-936e-d69eb3590068" & agri_6_28_1 == 0
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0
	replace ind_var = 1 if agri_income_01 == 1 & missing(agri_income_02)
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0
	replace ind_var = 1 if agri_income_02 == 3 & missing(agri_income_02_o)
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0
	replace ind_var = 1 if species_autre == 1 & missing(species_o)
...
forvalues i = 1/5{
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if agri_income_09_`i' == 7 & missing(agri_income_09_o_`i')
...
*** xvi. animals_sales_t should be answered when animals_sales_o = 1 ***
	...
	*** generate indicator variable ***
	gen ind_var = 0
	replace ind_var = 1 if animals_sales_o == 1 & missing(animals_sales_t)
...
forvalues i = 1/1 {
	...
	*** generate indicator variable ***
	gen ind_var = 0
	replace ind_var = 1 if agri_income_13_99_`i' == 1 & agri_income_13_99_`i' != . & missing(agri_income_13_autre_`i')
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if agri_income_15 == 1 & missing(agri_income_18)
...
preserve 
	...
	*** generate indicator variable ***
	...
	replace ind_var = 1 if agri_income_18 == 1 & missing(agri_income_18_o)
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if agri_income_30 == 1 & missing(agri_income_31)
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if agri_income_31_6 == 1 & missing(agri_income_31_o)
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if agri_income_34 == 1 & missing(agri_income_name)
...
*** check if animal_sales is missing ***
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if missing(animals_sales)
...
foreach var of varlist agri_income_22_o agri_income_22_1 agri_income_22_2 {
preserve
    keep if `var'  != . & `var' > 12
	generate issue = "Unreasonable value"
	generate issue_variable_name = "`var'"
...
forvalues i = 1/2 {
	...
	drop if key == "uuid:64a3cbc6-e8b8-4e8f-b508-bdb4d8a0abc9" & agri_income_23_1 == -9
	generate issue = "Issue found: unreasonable value"
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if living_01 == 99 & missing(living_01_o)
...
preserve 
	...
	*** generate indicator variable ***
	...
	gen ind_var = 0 
	replace ind_var = 1 if living_03 == 99 & missing(living_03_o)
...
preserve 
	...
	*** generate indicator variable ***
	...
	gen ind_var = 0 
	replace ind_var = 1 if living_04 == 99 & missing(living_04_o)
...
tostring living_05_o, replace 
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if living_05 == 99 & missing(living_05_o)
...
preserve 
	...
	*** generate indicator variable ***
	...
	replace ind_var = 1 if living_06 == 99 & missing(living_06_o)
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if enum_03 == 99 & missing(enum_03_o)
...
preserve 
	...
	*** generate indicator variable ***
	gen ind_var = 0 
	replace ind_var = 1 if enum_04 == 99 & missing(enum_04_o)
...
preserve 
	...
	*** generate indicator variable ***
	...
	gen ind_var = 0 
	replace ind_var = 1 if enum_05 == 99 & missing(enum_05_o)
