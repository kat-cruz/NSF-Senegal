...
# Example usage
f_test_results <- run_f_tests(df_wide, treatment_col = "group", vars_to_test = c("hh_15_2", "hh_26_", "hh_27_", "hh_30_", "hh_35_"))
print(f_test_results)

```

## Step 3: Function for Joint F-statistics
...
##Issue here - need to find better way to merge
...
# Create larger dataframe (4 rows, 6 columns)
df_large <- data.frame(
  A = c(1, 2, 3, 4),
  B = c(5, 6, 7, 8),
  C = c(9, 10, 11, 12),
...
# Create smaller dataframe (1 row, 4 columns)
df_small <- data.frame(
  A = 25,
  B = 26,
  C = 27,
...
# Print the original dataframes to check them
...
# Add the row from df_small to df_large, aligning with the first 4 columns
df_large_updated <- df_large %>%
  bind_rows(df_small %>% mutate(E = NA, F = NA))  # Adding missing columns (E and F)

# Print the updated dataframe
print("Updated Large Dataframe:")
print(df_large_updated)
...
df <- expand.grid(variable = variables, comparison = comparisons) %>%
  mutate(
    f_statistic = round(runif(n(), 3, 6), 2),  # Random F-statistics between 3 and 6
    p_value = round(runif(n(), 0, 0.05), 4),   # Random p-values between 0 and 0.05
    stat = round(rnorm(n(), mean = -2, sd = 1), 2), # Random stat values
    p = round(runif(n(), 0, 0.1), 4)  # Random p-values for the stat test
  )
...
```{r}
# Reshape data to wide format with "stat" and "p" as rows
df_wide <- df %>%
  pivot_wider(
  ...
  rename_with(~gsub(" ", "_", .), everything()) # Remove spaces in column names

# Move f_statistic and p_value to separate columns
df_final <- df_wide %>%
  select(variable, f_statistic, p_value, everything())
...
library(stats)   # For p-value adjustments
...
fake_data$y_treatment1 <- ifelse(fake_data$group == "Treatment1", 1, 0)
fake_data$y_treatment2 <- ifelse(fake_data$group == "Treatment2", 1, 0)
fake_data$y_treatment3 <- ifelse(fake_data$group == "Treatment3", 1, 0)
fake_data$y_control <- ifelse(fake_data$group == "Control", 1, 0)

# Run multinomial logistic regression (with control as baseline)
model <- multinom(cbind(y_treatment1, y_treatment2, y_treatment3, y_control) ~ value + variable, data = fake_data)
...
# Extract coefficients and p-values
#model_summary <- summary(model)$coefficients
model_p_values <- summary(model)$coefficients[, 4]  # p-values are typically in the 4th column of the coefficients matrix

q_values <- p.adjust(model_p_values, method = "fdr")
# q_values
# # Calculate FDR q-values for multiple hypothesis correction
#p_values <- as.vector(model_p_values)
# p_values
...
regression_results <- tibble(
  ...
  coef_treatment1 = summary(model)$coefficients[, 1],  # Coefficients for Treatment1
  coef_treatment2 = summary(model)$coefficients[, 2],  # Coefficients for Treatment2
  coef_treatment3 = summary(model)$coefficients[, 3],  # Coefficients for Treatment3
  coef_control = summary(model)$coefficients[, 4],     # Coefficients for Control
...
# Fit a model to investigate the effect of the number of cylinders on fuel economy
model <- aov(mpg ~ as.factor(cyl), data = mtcars)

# Summarize the model and get the ANOVA table with the F-statistic
summary(model)
...
set.seed(123)
data <- data.frame(
  transport = factor(sample(c("Car", "Bike", "Bus", "Train"), 200, replace = TRUE)),
...
model <- multinom(transport ~ income + age, data = data)
...
```{r}
# Install car package (if not ins

set.seed(123)

# Create a fake dataset with 200 observations
n <- 200
data <- data.frame(
...
# For Treatment2
joint_test_treatment2 <- linearHypothesis(m, "valueTreatment2 = 0")

# For Treatment3
joint_test_treatment3 <- linearHypothesis(m, "valueTreatment3 = 0")

# Print each result
print(joint_test_treatment1)
print(joint_test_treatment2)
print(joint_test_treatment3)
...
# Define parameters
num_villages <- 30
num_households <- 300  # Total households
treatment_arms <- c("Control", "Local Control", "Treatment1", "Treatment2", "Treatment3")

# Generate household and village IDs
df <- data.frame(
  target_village = sample(1:num_villages, num_households, replace = TRUE),  # Village-level clustering
  treatment_group = sample(treatment_arms, num_households, replace = TRUE)  # Random treatment assignment
)

# Generate baseline covariates
df <- df %>%
  mutate(
    beliefs_01_bin = rbinom(num_households, 1, 0.5),
    beliefs_02_bin = rbinom(num_households, 1, 0.5),
    beliefs_03_bin = rbinom(num_households, 1, 0.5),
    beliefs_04_bin = rbinom(num_households, 1, 0.5),
    beliefs_05_bin = rbinom(num_households, 1, 0.5),
    beliefs_06_bin = rbinom(num_households, 1, 0.5),
    beliefs_07_bin = rbinom(num_households, 1, 0.5),
    beliefs_08_bin = rbinom(num_households, 1, 0.5),
    beliefs_09_bin = rbinom(num_households, 1, 0.5),
    health_5_3_bin = runif(num_households, 0, 1),  # Continuous between 0 and 1
    health_5_6_ = runif(num_households, 0, 1),     # Continuous between 0 and 1
    num_water_access_points = sample(1:5, num_households, replace = TRUE),  # Discrete variable
    q_51 = rnorm(num_households, mean = 5, sd = 1)  # Normally distributed continuous variable
  )
df
# Convert 'treatment_group' into a factor with 'Control' as the reference group
df <- df %>%
  mutate(treatment_group = factor(treatment_group, levels = c("Control", "Local Control", "Treatment1", "Treatment2", "Treatment3")))
df


# ==========================
# 📌 Step 2: Balance Table via Regression with Clustered SEs
# ==========================

# List of baseline covariates to test
baseline_vars <- c("beliefs_01_bin", "beliefs_02_bin", "beliefs_03_bin", "beliefs_04_bin", 
                   "beliefs_05_bin", "beliefs_06_bin", "beliefs_07_bin", "beliefs_08_bin", 
                   "beliefs_09_bin", "health_5_3_bin", "health_5_6_", "num_water_access_points", 
                   "q_51")
# Load necessary package

# Function to run regression for each variable against the treatment group
run_regressions <- function(data, treatment_col, village_col, vars_to_test) {
  results <- list()
  
  # Loop over all variables to run regression
  for (var in vars_to_test) {
    treatments <- unique(data[[treatment_col]])
    treatments <- setdiff(treatments, 'Control')  # Remove control group for comparisons
    
    # Compare each treatment arm to the control
    for (arm in treatments) {
      # Subset data for control vs. arm comparison
      subset <- data %>% filter(.data[[treatment_col]] %in% c('Control', arm) & variable == var)
      subset$treat <- ifelse(subset[[treatment_col]] == arm, 1, 0)  # Create treatment indicator (1 for arm, 0 for control)
      
      # Run the regression (lm model)
      model <- lm(value ~ treat, data = subset)
      
      # Cluster standard errors by village
      clustered_se <- coeftest(model, vcov = vcovCL(model, cluster = subset[[village_col]]))
      
      # Store results (for control vs. arm comparison)
      results <- append(results, list(data.frame(
        variable = var,
        comparison = paste('Control vs', arm),
        stat = round(clustered_se[2, 1], 2),  # Coefficient (estimate)
        p = round(clustered_se[2, 4], 3)      # p-value
      )))
    }
  }
...
# Example: Run the regression for all variables in the dataset
# Replace `long_data` with your actual data frame
vars_to_test_reg <- setdiff(names(long_data), c("hhid", "hhid_village", "treatment_group", "variable"))
regression_results <- run_regressions(long_data, 'treatment_group', 'hhid_village', vars_to_test_reg)
...
# Function to run regressions for each variable against the treatment group
run_regressions <- function(data, treatment_col, village_col, vars_to_test) {
  results <- list()
  
  # Loop over all baseline variables to run regression
  for (var in vars_to_test) {
    treatments <- unique(data[[treatment_col]])
    treatments <- setdiff(treatments, 'Control')  # Remove control group for comparisons
    ...
    for (arm in treatments) {
      # Subset data for control vs. arm comparison
      subset <- data %>% filter(.data[[treatment_col]] %in% c('Control', arm))
      subset$treat <- ifelse(subset[[treatment_col]] == arm, 1, 0)  # Create treatment indicator (1 for arm, 0 for control)
      
      # Run the regression (lm model)
      model <- lm(as.formula(paste(var, "~ treat")), data = subset)  # Use formula interface
...
regression_results <- run_regressions(df, 'treatment_group', 'target_village', baseline_vars)
...
# Example model fitting with multinom (multinomial regression)
model <- multinom(treatment_group ~ beliefs_01_bin + beliefs_02_bin + beliefs_03_bin + 
                  beliefs_04_bin + beliefs_05_bin + beliefs_06_bin + beliefs_07_bin + 
                  beliefs_08_bin + beliefs_09_bin + health_5_3_bin + health_5_6_ + 
                  num_water_access_points + q_51, data = df)
...
# Perform the Wald test to check the joint significance of 'beliefs_01_bin' and baseline variables across the treatments
joint_test <- linearHypothesis(model, coefs)
