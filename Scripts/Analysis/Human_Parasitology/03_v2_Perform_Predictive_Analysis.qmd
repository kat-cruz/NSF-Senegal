...
format:
  html:
    ...
    theme: default
    output-file: "Parasitological_Analysisv3.html" #MAKE SURE TO UPDATE
execute:
  echo: true
---
...
paras_sum_stats_ad <- paras_sum_stats_ad %>%
  mutate(
    ...
    source = case_when(
      Variable %in% Variable[match("hh_age_", Variable):match("asset_index_std", Variable)] ~ "CRDES Household Survey Data",
      Variable == "q_51" | Variable == "q_51_w99" ~ "CRDES Community Survey Data",
      Variable %in% Variable[match("sh_inf", Variable):match("sex_hp", Variable)] ~ "EPLS/UCAD Parasitological Data",
...
# Prepare the base table (optional: you could keep section/source columns if you want to show them)
...
kbl_base %>%
  ...
  footnote(general = "Note: Baseline: 206 observations, Midline: 185 observations. Means are calculated from non-missing values.")
...
In the Prediction – all tab, can you please add a scatter plot of the predicted probability of infection against the egg count, perhaps color coding S.haem. and S.mans. differently to enable visual differentiation?
