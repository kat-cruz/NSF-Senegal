...
********************* UPDATE WITH DATE *****************************
export excel "$village_observations\Village_Observations.xlsx", replace firstrow(variables)
...
export excel "$village_observations\HouseholdIDs.xlsx", replace firstrow(variables)
restore 
...
forvalues i=1/55 {
	...
	*** drop no consent households *** 
	...
	replace ind_var = 0 if `hh_education_level' < 98
	replace ind_var = 0 if _household_roster_count < `i'
...
forvalues i=1/55 {
	...
	*** drop no consent households *** 
    ...
    gen ind_var = 0
    replace ind_var = 1 if `hh_education_level ' > `hh_age'
	replace ind_var = 0 if _household_roster_count < `i'
...
preserve
	...
	*** drop no consent households *** 
	...
	tostring(print_issue), replace
	keep villageid hhid sup enqu hh_phone hh_head_name_complet hh_name_complet_resp issue_variable_name issue
