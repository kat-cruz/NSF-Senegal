...
###Function for F-tests on Each Variable


```{r}

# Assuming your data is in the `df` dataframe
# Subset to include only the treatment arms and Local Control (excluding Pure Control)
df_subset <- df %>% filter(treatment_group %in% c("Local Control", "Treatment1", "Treatment2", "Treatment3"))

# Perform the ANOVA to compare the means of the treatment groups
anova_result <- aov(value ~ treatment_group, data = df_subset)

# Summary of the ANOVA to get the F-statistic and p-value
